...
class DevelopmentConfig(Config):
    ...
    SQLALCHEMY_DATABASE_URI = "sqlite:///" + \
        os.path.join(instance_folder, formed_dbname if formed_dbname else 'dev.db')
...
class TestingConfig(Config):
    ...
    SQLALCHEMY_DATABASE_URI = "sqlite:///" + \
        os.path.join(instance_folder, formed_dbname if formed_dbname else 'test.db')


class ProductionConfig(Config):
    FLASK_ENV = 'production'
    SQLALCHEMY_DATABASE_URI = os.getenv('PROD_DATABASE_URl', default="sqlite:///" + os.path.join(
        instance_folder, formed_dbname if formed_dbname else 'prod.db'))
